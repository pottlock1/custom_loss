...
def precision_recall_auc_loss(
    ...
    label_priors=None,
    surrogate_type='xent',
    lambdas_initializer=tf.compat.v1.constant_initializer(1.0),
    reuse=None,
  ...
  with tf.compat.v1.variable_scope(scope,
    ...
    maybe_log2 = tf.math.log(2.0) if surrogate_type == 'xent' else 1.0
    ...
    per_label_loss = delta * tf.reduce_sum(input_tensor=per_anchor_loss, axis=2)
...
def roc_auc_loss(
  ...
  Raises:
    ValueError: If `surrogate_type` is not `xent` or `hinge`.
  """
  with tf.compat.v1.name_scope(scope, 'roc_auc', [labels, logits, weights]):
    ...
    loss = tf.reduce_mean(input_tensor=tf.abs(labels_difference) * weighted_loss, axis=0) * 0.5
...
def _prepare_labels_logits_weights(labels, logits, weights):
  ...
  logits = tf.convert_to_tensor(value=logits, name='logits')
...
def true_positives_lower_bound(labels, logits, weights, surrogate_type):
  ...
  return tf.reduce_sum(input_tensor=weights * (labels - loss_on_positives), axis=0)
...
def false_positives_upper_bound(labels, logits, weights, surrogate_type):
  ...
  return tf.reduce_sum(input_tensor=weights *  loss_on_negatives, axis=0)
