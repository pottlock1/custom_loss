...
def build_label_priors(labels,
  ...
  try:
    ...
    weighted_label_counts_update = weighted_label_counts.assign_add(
        tf.reduce_sum(input_tensor=weights * labels, axis=0))
    ...
    weight_sum = tf.compat.v1.get_variable(
        ...
        collections=variables_collections,
        trainable=False)
    weight_sum_update = weight_sum.assign_add(tf.reduce_sum(input_tensor=weights, axis=0))
...
def convert_and_cast(value, name, dtype):
  ...
  Returns:
    A tensor.
  """
  return tf.cast(tf.convert_to_tensor(value=value, name=name), dtype=dtype)
